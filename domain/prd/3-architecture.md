...
```
製程順序：S01 錫膏印刷 → S02 回流焊 → S03 AOI 光學檢測 → S04 電測／最終組裝

PENDING
  → [UNIT_LOAD @ S01] → S01_IN_PROGRESS
  → [UNIT_UNLOAD @ S01] → S01_DONE
  → [UNIT_LOAD @ S02] → S02_IN_PROGRESS
  → [UNIT_UNLOAD @ S02] → S02_DONE
  → [UNIT_LOAD @ S03] → S03_IN_PROGRESS
  → [UNIT_UNLOAD @ S03 / AOI_PASS] → S03_DONE
  → [UNIT_UNLOAD @ S03 / AOI_FAIL] → PENDING_REVIEW
      → [REVIEW_PASS] → S03_DONE
      → [REVIEW_FAIL] → REJECTED
  → [UNIT_LOAD @ S04] → S04_IN_PROGRESS
