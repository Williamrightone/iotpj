...
class UpdateMachineUseCaseTest {

    @Mock IotCoreFeignClient iotCoreClient;
    @InjectMocks UpdateMachineUseCase useCase;
    ...
    @Test
    void execute_updates_machine_for_admin() {
        UserContextHolder.set(ctx(Role.ADMIN));
        UpdateMachineRq rq = new UpdateMachineRq("New Name", "NewModel");
        ...
        given(iotCoreClient.updateMachine(1L, 10L, rq)).willReturn(response);
    ...
    void execute_viewer_throws_FORBIDDEN() {
        ...
        assertThatThrownBy(() -> useCase.execute(1L, new UpdateMachineRq("n", null)))
