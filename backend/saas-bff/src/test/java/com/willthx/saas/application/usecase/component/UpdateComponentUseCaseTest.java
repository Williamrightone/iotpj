...
class UpdateComponentUseCaseTest {

    @Mock IotCoreFeignClient iotCoreClient;
    @InjectMocks UpdateComponentUseCase useCase;
    ...
    @Test
    void execute_updates_component_for_admin() {
        UserContextHolder.set(ctx(Role.ADMIN));
        UpdateComponentRq rq = new UpdateComponentRq("Updated", "kPa", 30, null, null);
        ...
        given(iotCoreClient.updateComponent(1L, 10L, rq)).willReturn(response);
    ...
    void execute_viewer_throws_FORBIDDEN() {
        ...
        assertThatThrownBy(() -> useCase.execute(1L, new UpdateComponentRq("n", null, null, null, null)))
